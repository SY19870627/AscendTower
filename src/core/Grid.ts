...
export class Grid {
w:number; h:number; tiles: Tile[][]; rng: RNG
keyPos!: Vec2; doorPos!: Vec2; stairsPos!: Vec2; playerPos!: Vec2
enemyPos: Vec2[] = []

constructor(w=11,h=11, seed=1337){
this.w=w; this.h=h; this.rng = new RNG(seed)
...
generate(){
// �~����
for(let y=0;y<this.h;y++) for(let x=0;x<this.w;x++) if(x===0||y===0||x===this.w-1||y===this.h-1) this.tiles[y][x]='wall'
// ²�椺���H��
for(let i=0;i<18;i++){ const x=this.rng.int(1,this.w-2), y=this.rng.int(1,this.h-2); this.tiles[y][x]='wall' }
// ��m���䪫��G���B�_�͡B�ӱ�B���a�B�ĤH
this.playerPos = this.place('player')
this.keyPos = this.place('key')
this.doorPos = this.place('door')
this.stairsPos = this.place('stairs')
// ?�H 5 ��
for(let i=0;i<5;i++){ this.enemyPos.push(this.place('enemy')) }
// �T�O��¦�s�q�]²�ơG�ʼɲM���^
...
if(this.tiles[p.y][p.x]==='floor'){ this.tiles[p.y][p.x]=t; return p }
}
}

carvePath(a:Vec2,b:Vec2){
const dx=Math.sign(b.x-a.x), dy=Math.sign(b.y-a.y)
let x=a.x, y=a.y
while(x!==b.x){ x+=dx; if(this.tiles[y][x]==='wall') this.tiles[y][x]='floor' }
